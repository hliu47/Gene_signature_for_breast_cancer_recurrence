rf_my = list(
  ...
  fit = function (x, y, wts, param, lev, last, classProbs, ...) {
    # cat('ntree is', rf_tree_n, '\n')
    randomForest::randomForest(x, y, ntree = rf_tree_n, mtry = param$mtry, ...)
  },
  
  predict = function (modelFit, newdata, submodels = NULL) if (!is.null(newdata)) predict(modelFit, newdata) else predict(modelFit),
  prob = function (modelFit, newdata, submodels = NULL) if (!is.null(newdata)) predict(modelFit, newdata, type = "prob") else predict(modelFit, type = "prob"),
